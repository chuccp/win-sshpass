...
> 跨平台 sshpass 實作（Windows 和 Linux）：密碼/金鑰 SSH 登入、互動式 Shell、SFTP 檔案傳輸、SCP/Rsync 風格傳輸、代理通道、斷點續傳、檔案雜湊校驗，以及可重複使用的 Go SDK。

## 什麼是 win-sshpass？

win-sshpass 是 Linux sshpass 工具的跨平台實作，支援 Windows 和 Linux。它是一個獨立的可執行檔，無需安裝 OpenSSH 或其他軟體，下載即可使用。
