...
> クロスプラットフォーム版 sshpass（Windows および Linux）：パスワード/キー SSH ログイン、インタラクティブシェル、SFTP ファイル転送、SCP/Rsync スタイル転送、プロキシトンネル、ブレークポイントレジューム、ファイルハッシュ検証、そして再利用可能な Go SDK。

## win-sshpass とは？

win-sshpass は Linux の sshpass ツールのクロスプラットフォーム実装です。Windows と Linux で動作する独立した実行ファイルで、OpenSSH や他のソフトウェアのインストールは不要です。ダウンロードしてすぐに使えます。
...
| `WithResume()` | ファイル転送のブレークポイントレジュームを有効化 |
