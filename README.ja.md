...
- **Dockerテスト** — ローカルDockerコンテナを使用した包括的統合テストスイート
...
ローカルの統合テストスイートが Docker コンテナに対して実行され、全機能をカバーします：
...
Docker イメージに含まれるもの：
- パスワード認証と鍵認証に対応した OpenSSH サーバー
- シェルファイル転送テスト用の lrzsz（rz/sz）
- rsync スタイルの転送テスト用の rsync
- 事前にデプロイされたテスト用 Ed25519 鍵ペア
- テストユーザー：`testuser`/`testpass` および `root`/`rootpass`
- `/tmp/testdata/` 内のテストデータファイル
