...
> Windows 平台的 sshpass 实现：密码/密钥 SSH 登录、交互式 Shell、SFTP 文件传输、SCP/Rsync 风格传输，以及可复用的 Go SDK。

## 什么是 win-sshpass？

win-sshpass 是 Linux sshpass 工具的 Windows 实现。它是一个独立的可执行文件，无需安装 OpenSSH 或其他依赖，下载即用。
...
```bash
# 下载 win-sshpass.exe（或通过 Scoop 安装）
